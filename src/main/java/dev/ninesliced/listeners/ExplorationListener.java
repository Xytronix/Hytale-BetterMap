...
public class ExplorationListener {
	...
	/**
	 * Handles the PlayerReadyEvent. Initializes tracking if the player joins the
	 * default world.
	 *
	 * @param event
	 *            The event.
	...
	public static void onPlayerReady(@Nonnull PlayerReadyEvent event) {
		try {
					...
					try {
						if (player.getReference() != null && player.getReference().isValid()) {
							player.sendMessage(Message.raw(
									"WARNING: BetterMap - Just added this world as tracked but you need to restart the server to apply the changes.")
									.color(Color.RED));
						}
					} catch (Exception e) {
				...
				if (trackedWorld != null && trackedWorld.equals(currentWorld)) {
					if (isTrackedWorld(world)) {
						if (ExplorationTracker.getInstance().getPlayerData(playerName) != null) {
							LOGGER.info("[DEBUG] Player " + playerName + " already tracked in world " + currentWorld
									+ ", skipping PlayerReadyEvent");
							return;
						}
					}
				}
			...
			if (isTrackedWorld(world)) {
				...
				LOGGER.info("[DEBUG] Loaded exploration data for " + playerName + ", explored chunks: "
						+ explorationData.getExploredChunks().getExploredChunks().size());
				...
				if (PermissionsUtil.isAdmin(player)) {
					ExplorationTicker.getInstance().scheduleDelayedTask(() -> {
						try {
							if (player.getReference() != null && player.getReference().isValid()) {
								player.sendMessage(
										Message.raw("WARNING: BetterMap - usage in this world is not tracked.")
												.color(Color.RED));
								player.sendMessage(Message.raw("Use '/bettermap config track' to track this world.")
										.color(Color.RED));
							}
						} catch (Exception e) {
							LOGGER.warning("Failed to send warning message: " + e.getMessage());
	...
	/**
	 * Handles the DrainPlayerFromWorldEvent. Saves data and cleans up tracking when
	...
	/**
	 * Handles the AddPlayerToWorldEvent. Manages world transitions, saving old data
	 * and loading new data if applicable. Uses synchronized operations to prevent
	 * race conditions during rapid world changes.
	 *
	 * @param event
	 *            The event.
	 */
	public static void onPlayerJoinWorld(@Nonnull AddPlayerToWorldEvent event) {
		...
		try {
			...
			WaypointLimitUtil.applyOverridesToWorld(newWorld, config.getMaxPersonalMarkersPerPlayer(),
					config.getMaxSharedMarkersPerPlayer());

			LOGGER.info("[DEBUG] Player " + playerName + " joining world: " + newWorldName + " (previous: "
					+ oldWorldName + ")");

			if (oldWorldName != null && !oldWorldName.equals(newWorldName)) {
				LOGGER.info("[DEBUG] WORLD CHANGE DETECTED: " + playerName + " from " + oldWorldName + " to "
						+ newWorldName);
			...
			if (!isTrackedWorld(newWorld)) {
				...
				ExplorationTracker.PlayerExplorationData newData = ExplorationTracker.getInstance()
				...
				ExplorationTicker.getInstance().scheduleUpdate(() -> {
					...
					if (currentWorld == null || !currentWorld.getName().equals(finalNewWorldName)) {
						LOGGER.info(
								"[DEBUG] Player " + playerName + " already changed worlds, skipping scheduled update");
						return;
					}
	...
	/**
	 * Handles the PlayerDisconnectEvent. Ensures final data save on disconnect.
	 *
	 * @param event
	 *            The event.
	 */
			...
			if (data != null) {
				...
				if (fallbackRef != null && fallbackRef.isValid()) {
					try {
						...
						if (isTrackedWorld(world)) {
							LOGGER.info("[DEBUG] Fallback save for player " + playerName
									+ " disconnecting from default world");
	...
	/**
	 * Initializes the dynamic cave mode for a player (creates their state). The new
	 * system is automatic - no need to restore from config.
	 *
	 * @param player
	 *            The player.
	 */
	private static void initDynamicCaveMode(@Nonnull Player player) {
		try {
			...
			LOGGER.warning(
					"Failed to initialize dynamic cave mode for " + player.getDisplayName() + ": " + e.getMessage());
		}
	}
	...
	/**
	 * Cleans up cave mode state for a player by name. Use this when the Player
	 * object might not be available (e.g., on disconnect).
	 *
	 * @param playerName
